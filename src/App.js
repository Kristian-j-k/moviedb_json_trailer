import { Routes, Route, Link } from 'react-router-dom';
import HighestRatedMovieList from "./pages/highestRatedMoviesList";
import PopularMoviesList from "./pages/popularMoviesList";
import MovieImg from './assets/Image/movie_black2.jpg';
import Home from './pages/home';
import SearchedMoviesList from './pages/searchedMovieList';
import SearchMovie from './components/searchMovie';
import FavoriteMoviesList from './pages/FavoriteMovieList';

function App() {
  return (
    <div>
      <div className="jumbotron pb-3 pt-3">
        <div className="navbar navbar-expand-lg">
          <nav className="nav navbar-nav">    
            <Link to='/' className="nav-item nav-link">Home</Link>
            <Link to='/popular' className="nav-item nav-link">Popular</Link>
            <Link to='/highest-rated' className="nav-item nav-link">Highest Rated</Link>
            <Link to='/favorites' className="nav-item nav-link">Favorites</Link>
          </nav>
          
        </div>
          <span className='h1'>React Moviefinder <img className="rounded movie_img m-3" src={MovieImg} width="75" height="75" alt=''/></span>
      <span className="d-flex justify-content-between p-0">This small App demonstrates React, Redux-Toolkit, RTK Query and React-Router</span>
      <SearchMovie/>
        </div>
        <Routes>
            <Route path='/' element={<Home/>} />  
            <Route path='/popular' element={<PopularMoviesList/>} />    
            <Route path='/highest-rated' element={<HighestRatedMovieList/>} />
            <Route path='/searchedMovieList' element={<SearchedMoviesList/>} />
            <Route path='/favorites' element={<FavoriteMoviesList/>} />
            
        </Routes>
        
        
    </div>
  );
}
  export default App;